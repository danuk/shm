...
(default,40092,'manual',455.00,'2014-10-02 14:05:39',''),
(default,40092,'manual',455.00,'2016-01-04 20:33:35','');
...
(1,'Тариф \"Почтовый\" (${QUOTA} мб)',75,0,'web_tariff_lock','[8]',NULL,NULL,NULL,NULL,1,NULL,'Тарифный план включает в себя набор услуг. Все услуги, включенные в тарифный план, полностью зависят от статуса тарифа.',NULL,NULL),
...
(11,'Регистрация домена в зоне .RU: ${DOMAIN}',590,12,'domain','[30,31]',12,NULL,NULL,NULL,1,1,'Регистрация домена осуществляется регистратором доменных имен.',NULL,NULL),
...
(40092,0,'danuk','d8923baf143645690cc89db46e4611fb1066e1f0',0,'2014-09-30 14:17:37',NULL,0,-21.56,30,100000.00,NULL,NULL,0,0,0,0,'Фирсов Даниил Андреевич',0,100.00,NULL,NULL,NULL),
...
('wg_manager','#!/bin/bashset -eEVENT=\"{{ event_name }}\"WG_MANAGER=\"/etc/wireguard/wg-manager.sh\"SESSION_ID=\"{{ user.gen_session.id }}\"# We need the --fail-with-body option for curl.# It has been added since curl 7.76.0, but almost all Linux distributions do not support it yet.# If your distribution has an older version of curl, you can use it (just comment CURL_REPO)CURL_REPO=\"https://github.com/moparisthebest/static-curl/releases/download/v7.86.0/curl-amd64\"CURL=\"/opt/curl/curl-amd64\"#CURL=\"curl\"echo \"EVENT=$EVENT\"case $EVENT in    INIT)        SERVER_HOST=\"{{ server.settings.host_name }}\"        SERVER_INTERFACE=\"{{ server.settings.host_interface }}\"        if [ -z $SERVER_HOST ]; then            echo \"ERROR: set variable \'host_name\' to server settings\"            exit 1        fi        apt update        apt install -y \\            wireguard \\            wireguard-tools \\            qrencode \\            wget        if [[ $CURL_REPO && ! -f $CURL ]]; then            mkdir -p /opt/curl            cd /opt/curl            wget $CURL_REPO            chmod 755 $CURL        fi        cd /etc/wireguard        $CURL -s --fail-with-body https://danuk.github.io/wg-manager/wg-manager.sh > $WG_MANAGER        chmod 700 $WG_MANAGER        if [ $SERVER_INTERFACE ]; then            $WG_MANAGER -i -s $SERVER_HOST -I $SERVER_INTERFACE        else            $WG_MANAGER -i -s $SERVER_HOST        fi        ;;    CREATE)        USER_CFG=$($WG_MANAGER -u \"{{ us.id }}\" -c -p)        $CURL -s --fail-with-body -XPUT \\            -H \"session-id: $SESSION_ID\" \\            -H \"Content-Type: text/plain\" \\            {{ config.api.url }}/shm/v1/storage/manage/vpn{{ us.id }} \\            --data-binary \"$USER_CFG\"        echo \"done\"        ;;    ACTIVATE)        $WG_MANAGER -u \"{{ us.id }}\" -U        echo \"done\"        ;;    BLOCK)        $WG_MANAGER -u \"{{ us.id }}\" -L        echo \"done\"        ;;    REMOVE)        $WG_MANAGER -u \"{{ us.id }}\" -d        $CURL -s --fail-with-body -XDELETE \\            -H \"session-id: $SESSION_ID\" \\            {{ config.api.url }}/shm/v1/storage/manage/vpn{{ us.id }}        echo \"done\"        ;;    *)        echo \"Unknown event: $EVENT. Exit.\"        exit 0        ;;esac',NULL),
