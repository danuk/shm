...
is( $ret->{ $us->id }->{withdraws}->{total}, '1000.00', 'Check total for one month' );
...
is( $ret->{ $us->id }->{withdraws}->{total}, '360.00', 'Check total for 4 months with discount' );
...
is( $ret->{ $us->id }->{withdraws}->{total}, '1000.00', 'Check total for domain' );
